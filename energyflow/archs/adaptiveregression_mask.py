...
class SymmetricPerParticleNN(NNBase):
    ...
    def _process_hps(self):
        ...
        **Default EFN Hyperparameters**
        ...
        self.Phi2_sizes = self._proc_arg('Phi2_sizes', old='ppm_sizes')
    ...
    def _construct_model(self):
        ...
        if self.use_moment_info == True:
            ...
            observables= phi_tensor
        group_layer = Dense(observables.get_shape().as_list()[-1], name='weights')
        matrix = group_layer(self.tensors[-1]) 

        self._tensors.append(squeeze(matmul(expand_dims(expand_dims(matrix, axis=1),axis=1), expand_dims(observables, axis=-1), transpose_a=False), axis=-1))
        sigmoid = _get_act_layer('sigmoid')
        mask_tensor = sigmoid(squeeze(self.tensors[-1], axis=-1))
        self._weights =  mask_tensor*self.weights
        self._construct_Phi2(tensor_list=self.phi_inputs)
        latent_dim = self.tensors[-1].get_shape().as_list()[-1]
        self._construct_Phi_moment(latent_dim, self.order)
        self._construct_latent(n=2)
        self._construct_Phi_cumulant(latent_dim, self.order)
        self._construct_F(n=2)
    ...
    def _construct_Phi(self, tensor_list):
        ...
        self._layer_inds['Phi'] = layer_inds
        self._tensor_inds['Phi'] = tensor_inds
    def _construct_Phi2(self, tensor_list):
        ...
        for k, tensor in enumerate(tensor_list):
            names = [self._proc_name('iterated_tdist{}_{}'.format(k, i)) for i in range(len(self.Phi2_sizes))]
            if type(self.Phi2_sizes[-1]) == int:
                sizes = self.Phi2_sizes
            else:
                sizes = self.Phi2_sizes[:-1] + [self.Phi2_sizes[-1][k]]
            Phi_layers, Phi_tensors = construct_distributed_dense(tensor, sizes, 
                                                                acts=self.Phi_acts, 
                                                                k_inits=self.Phi_k_inits, 
                                                                names=names, 
    ...
    def _construct_latent(self, n=1):
        ...
        latent_layers, latent_tensors = construct_latent(self.tensors[-1], self.weights, 
                                                         ...
                                                         name=self._proc_name('sum{}'.format(n)))
    ...
    def _construct_F(self, n=1):

        # get names
        names = [self._proc_name('dense{}_{}'.format(n,i)) for i in range(len(self.F_sizes))]
