...
class SymmetricPerParticleNN(NNBase):
    ...
    def _construct_model(self):
        ...
        phi_input = self.tensors[-1]
        self._construct_Phi(input_tensor=phi_input, n=0)
        phi0 = self.tensors[-1]
        self._construct_Phi(input_tensor=phi_input, n=1)
        phi1 = self.tensors[-1]
        self._construct_Phi(input_tensor=phi_input, n=2)
        phi2 = self.tensors[-1]
        self._tensors.append(phi0*phi1*phi2)
    ...
    def _construct_Phi(self,input_tensor, n):

        # get names
        names = [self._proc_name('tdist{}_{}'.format(n,i)) for i in range(len(self.Phi_sizes))]

        # determine begin inds
        layer_inds, tensor_inds = [len(self.layers)], [len(self.tensors)]

        # construct Phi
        Phi_layers, Phi_tensors = construct_distributed_dense(input_tensor, self.Phi_sizes, 
                                                              acts=self.Phi_acts, 
